...
int main(int argc, char **argv){
    ...
    serv_addr.sin_port = htons(4321);  //We specify the port we want to connect to --> networks use big endian --> htons = host to networks 
                                        //So htons is converting the number to big endian
    serv_addr.sin_addr.s_addr = inet_addr("192.168.1.6"); // the ip address that you want to connect to
